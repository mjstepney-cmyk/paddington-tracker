...
TARGET_DESTINATIONS = ["PGN", "NTA", "PLY", "PNZ", "EXD", "BRI","CDF"]  # Paignton, Newton Abbot, Plymouth, Penzance, Exeter, Bristol, Cardiff
FROM_CRS = "PAD"
TD_AREA = "D3"
            ...
            with state_lock:
                for msg in messages:
                    if "CA_MSG" in msg:
                        ...
                        state["last_td_update"] = now_london()
                    elif "CB_MSG" in msg:
                        d = msg["CB_MSG"]
                        if d.get("area_id") != TD_AREA:
                            continue
...
def darwin_query():
    ...
    for dest in TARGET_DESTINATIONS:
        try:
            ...
            with state_lock:
                state["darwin_last_response"] = r.text[:2000]
...
def darwin_poll():
    while True:
        ...
        with state_lock:
            state["services"] = svcs
            state["last_darwin_update"] = now_london()
        time.sleep(300)
...
{% if not services %}
  <div class="no-services">No services to Paignton / Newton Abbot / Plymouth / Penzance in the next 4 hours.</div>
{% else %}
  {% for svc in services %}
  <div class="card">
...
def darwin_debug():
    with state_lock:
        return {
            "last_response": state["darwin_last_response"],
            "last_error": state["darwin_last_error"],
